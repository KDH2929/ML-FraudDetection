"""
Member A 전처리 v4 (슬라이드 기준) — A3 파생변수 위에 변수 압축 및 최적화를 얹은 확장.

성능 향상보다 불필요 변수 제거와 차원 축소에 초점:
- **다중공선성 제거**: `cust_claim_*` 초고상관 변수 드롭 (피어슨 0.99↑)
- **PCA 차원 압축**: 수치형 변수를 StandardScaler → PCA로 압축 (기본 n_components=10)

두 방법 모두 F1 변화는 미미 — 모델 복잡도 축소 목적.

선택적으로 `advanced_feature_selection=True` 를 주면, 상수·상관·VIF, f/chi2 필터,
RF `SelectFromModel`, LR-RFE 를 실험용으로 추가 적용한다.
...
from src.preprocessing.strategies.book.book_cust_claim_dedup_strategy import (
    _DROP_FOR_MULTICOLLINEARITY,
)
from .member_a_strategy_3 import MemberA4Strategy


def _drop_for_multicollinearity(df: pd.DataFrame) -> pd.DataFrame:
    drop = [c for c in _DROP_FOR_MULTICOLLINEARITY if c in df.columns]
    if drop:
...
def _apply_pca(
    ...
    *,
    n_components: int = 10,
    pca_fitted: PCA | None = None,
    scaler_fitted: StandardScaler | None = None,
) -> tuple[pd.DataFrame, PCA, StandardScaler]:
    """수치형 변수에 StandardScaler → PCA를 적용. 기존 컬럼은 유지하고 ma4_pca_* 추가."""
    meta = {ID_COL, DIVIDED_SET_COL}
    num_cols = [c for c in df.columns if c not in meta and pd.api.types.is_numeric_dtype(df[c])]
    if not num_cols:
        dummy_pca = PCA(n_components=min(n_components, 1))
        dummy_sc = StandardScaler()
        return df, dummy_pca, dummy_sc
    ...
    n_use = min(n_components, mat.shape[1], mat.shape[0])
    if n_use < 1:
        dummy_pca = PCA(n_components=1)
        dummy_sc = StandardScaler()
        return df, dummy_pca, dummy_sc

    if scaler_fitted is None:
        sc = StandardScaler()
        sc.fit(mat)
    else:
        sc = scaler_fitted
    ...
    if pca_fitted is None:
        ...
        pca.fit(sc.transform(mat))
    else:
        pca = pca_fitted

    comps = pca.transform(sc.transform(mat))
    out = df.copy()
    for j in range(comps.shape[1]):
        out[f"ma4_pca_{j}"] = comps[:, j].astype(np.float64)
    return out, pca, sc
...
class MemberA5Strategy(BaseStrategy):
    """
    A4(파생변수) + 변수 압축 최적화 (다중공선성 제거 + PCA).
    성능 향상보다 모델 복잡도 축소 목적.

    with_pca=False: PCA 생략 (다중공선성 제거만).
    top_k 설정 시: CorrelationRemover + LightGBM top-k 피처 선택 추가.
    ...
    def __init__(
        ...
        *,
        ...
        skip_corr_drop: bool = False,
        skip_paper_drop: bool | None = None,
    ) -> None:
        self._base = MemberA4Strategy()
        self._with_pca = with_pca
        ...
        self._skip_corr_drop = (
            skip_corr_drop if skip_paper_drop is None else bool(skip_paper_drop)
        )

    def get_strategy_name(self) -> str:
        parts = ["A5(v4): A4"]
        if not self._skip_corr_drop:
            parts.append("다중공선성 제거")
        if self._with_pca:
            parts.append(f"PCA({self._n_pca})")
    ...
    def preprocess(
        ...
        if not self._skip_corr_drop:
            out = _drop_for_multicollinearity(out)
        if self._with_pca:
            out, _, _ = _apply_pca(out, n_components=self._n_pca)
        if self._top_k is not None:
    ...
    def preprocess_train_test(
        ...
        if not self._skip_corr_drop:
            tr = _drop_for_multicollinearity(tr)
            te = _drop_for_multicollinearity(te)
            te = te.reindex(columns=tr.columns, fill_value=0.0)
        if self._with_pca:
            tr, pca_fit, sc_fit = _apply_pca(tr, n_components=self._n_pca)
            te, _, _ = _apply_pca(te, n_components=self._n_pca, pca_fitted=pca_fit, scaler_fitted=sc_fit)
            te = te.reindex(columns=tr.columns, fill_value=0.0)
        if self._top_k is not None:
            state = _fit_lgbm_top_k(
